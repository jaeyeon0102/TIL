...
def process_finished_tasks(time, desks, is_repair):
    ...
    '''
    # 이번 시간에 완료된 고객 수 초기화
    finished_customers_count = 0  
    # 접수를 마치고 정비를 기다리는 고객 목록
    newly_waiting_customers = []  

    # 데스크 길이만큼 순회
    for i in range(len(desks)):
        # 만약 해당 데스크에 고객이 있고, 해당 종료 시간과 현재 시간이 동일하다면
        if desks[i][0] > 0 and desks[i][1] == time:  # 현재 시간에 작업이 끝나는 고객이 있다면
            # 해당 고객 아이디 반환 및 창구에서 내보내기
